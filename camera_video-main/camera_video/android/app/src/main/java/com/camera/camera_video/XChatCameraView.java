...
public class XChatCameraView extends FrameLayout implements TextureView.SurfaceTextureListener {
    private MethodChannel mMethodChannel;

    public XChatCameraView(@NonNull Context context) {
        this(context, null);
    }

    public XChatCameraView(@NonNull Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public XChatCameraView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
            ...
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    if (isRecording) {
                        ...
                        if (moveY + 15 < curMoveY) {//往上移动，放大
                            curMoveY = moveY;
                            handleZoom(true);
                        } else if (moveY - 15 > curMoveY) {//往上移动，缩小
                            curMoveY = moveY;
                            handleZoom(false);
                        }
                    }
...
//        configureTransform(width, height);
    ...
    private void setupCamera() {
        ...
        try {
            ...
            Size tempSize = Camera2Util.getLowOptimalPreviewSize(map.getOutputSizes(SurfaceTexture.class), display.getWidth(), display.getHeight());
            if (tempSize == null) {
...
//            }
            mVideoPreviewSize = mPreviewSize;
            //此ImageReader用于拍照所需
            setupImageReader();
...
//            mPreviewBuilder.set(CaptureRequest.CONTROL_AWB_MODE, CaptureRequest.CONTROL_AWB_MODE_AUTO);
    ...
    private void stopMediaRecorder() {
        ...
        try {
            ...
            if (currentTime <= MIN_RECORD_TIME) {
                Toast.makeText(MainActivity.mContext, "录制时间过短", Toast.LENGTH_LONG).show();
    ...
    private void initZoomParameter() {
        ...
        float max_digital_zoom = characteristics.get(CameraCharacteristics.SCALER_AVAILABLE_MAX_DIGITAL_ZOOM);
